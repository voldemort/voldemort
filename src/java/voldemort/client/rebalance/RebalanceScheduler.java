...
// TODO: This scheduler deprecates the need for donor permits. Consider removing them.
/**
 * Scheduler for rebalancing tasks. There is at most one rebalancing task per stealer-donor pair.
 * This scheduler ensures the following invariant is obeyed:
 * 
 * A node works on no more than one rebalancing task at a time.
 * 
 * Note that a node working on a rebalancing task may be either a stealer or a donor. This invariant
 * should somewhat isolate the foreground workload against the work a server must do for
 * rebalancing. Because of this isolation, it is safe to attempt "infinite" parallelism since no
 * more than floor(number of nodes / 2) rebalancing tasks can possibly be scheduled to execute while
 * obeying the invariant.
 * 
 * The order of tasks are randomized within this class. The intent is to "spread" rebalancing work
 * smoothly out over the cluster and avoid "long tails" of straggler rebalancing tasks. Only
 * experience will tell us if we need to do anything smarter.
 */
public class RebalanceScheduler {
     ...
     * 
     * @param sbTaskList List of all stealer-based rebalancing tasks to be scheduled.
     */
     ...
     * 
     * @param boolean flag to control execution of the service
    ...
    protected synchronized StealerBasedRebalanceTask scheduleNextTask(boolean executeService) {
        ...
        if (this.numTasksExecuting == maxParallelRebalancing)
            return null;
        // Shuffle list of stealer IDs each time a new task to schedule needs to be found.
        // Randomizing the order should avoid prioritizing one specific stealer's work ahead of all
        // others.
        ...
        for (int stealerId : stealerIds) {
            ...
            for (StealerBasedRebalanceTask sbTask : tasksByStealer.get(stealerId)) {
                ...
                numTasksExecuting++;
                // Remove this task from list thus destroying list being iterated over.
                // This is safe because returning directly out of this branch.
    ...
    public synchronized void doneTask(int stealerId, int donorId) {
        ...
        // Try and schedule more tasks now that resources may be available to do so.
        scheduleMoreTasks();
    }
}