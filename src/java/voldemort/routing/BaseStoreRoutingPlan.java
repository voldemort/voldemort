...
public class BaseStoreRoutingPlan {
    ...
    private void verifyClusterStoreDefinition() {
        ...
        if (clusterZoneIds.size() > 1) { // Zoned
            ...
            if (!clusterZoneIds.equals(storeDefZoneIds)) {
                throw new VoldemortException("Zone IDs in cluster ("
                        + clusterZoneIds
     ...
     * 
     * @param partitionIds
     *            List of partition IDs for which to find the Node ID for the
     *            Node that owns the partition.
     * @return List of node ids, one for each partition ID in partitionIds
     * @throws VoldemortException
     *             If multiple partition IDs in partitionIds map to the same
     *             Node ID.
     */
    private List<Integer>
            getNodeIdListForPartitionIdList(List<Integer> partitionIds) throws VoldemortException {
        List<Integer> nodeIds = new ArrayList<Integer>(partitionIds.size());
        for (Integer partitionId : partitionIds) {
            int nodeId = getNodeIdForPartitionId(partitionId);
            if (nodeIds.contains(nodeId)) {
                throw new VoldemortException("Node ID " + nodeId
                        + " already in list of Node IDs.");
            } else {
                nodeIds.add(nodeId);
            }
        }
     ...
     */
    public List<Integer>
            getReplicationNodeList(int partitionId) throws VoldemortException {
    ...
    public int getZoneNAry(int zoneId, int nodeId, byte[] key) {
        if (cluster.getNodeById(nodeId).getZoneId() != zoneId) {
            throw new VoldemortException("Node " + nodeId + " is not in zone "
                    + zoneId + "! The node is in zone "
        ...
        if (zoneNAry > -1) {
            throw new VoldemortException("Node " + nodeId
                    + " not a replica for the key "
    ...
    public int getZoneNaryForNodesPartition(int zoneId,
        ...
        if (zoneNAry > 0) {
            throw new VoldemortException("Node " + nodeId
                    + " not a replica for partition " + partitionId
                    + " in given zone " + zoneId);
        } else {
     ...
     * 
     * @param keyPartitions
     *            Preference list of the key
     * @param nodePartitions
     *            Partition list on this node
     ...
     * 
     * @param nodeId
     *            Node on which this is running ( generally stealer node )
     * @param key
     *            The key to check
     * @param replicaToPartitionList
     *            Mapping of replica type to partition list
    ...
    public static boolean
        ...
        if (storeDef.getRoutingStrategyType()
            ...
            List<Integer> nodePartitions = cluster.getNodeById(nodeId)
                                                  .getPartitionIds();
     ...
     * @param key
     *            Byte array of key
     * @param stealerNodeToMappingTuples
     *            Pairs of stealer node id to their corresponding [ partition -
     *            replica ] tuples
     * @param cluster
     *            Cluster metadata
     * @param storeDef
