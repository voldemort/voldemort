...
public class SchemaEvolutionValidator
{
   ...
   * <li>ERROR messages indicate the schemas are not backward compatible and the new schema should not
   * be allowed. If an ERROR schema is uploaded, clients will not be able to read existing data.
   * <li>WARN messages indicate that the new schemas may cause problems for existing clients. However,
   * once all clients are updated to this version of the schema they should be able to read new and
   * existing data.
   * <li>INFO messages note changes to the schema, basically providing a friendly list of what has changed
   * from one version to the next. This includes changes like the addition of fields, changes to default
   * values, etc.
   *
   ...
   */
  public static List<Message> checkBackwardCompatability(Schema oldSchema, Schema newSchema, String name)
  {
  ...
  /* package private */ void compareTypes(Schema oldSchema, Schema newSchema, List<Message> messages, String name)
  {
    ...
    if(oldType != Type.UNION && newType == Type.UNION)
    {
      ...
      for(Schema s : newSchema.getTypes())
      {
        if( (oldType != Type.RECORD && oldType == s.getType()) ||
            (oldType == Type.RECORD && s.getType() == Type.RECORD && oldSchema.getName().equals(s.getName())))
        {
  ...
  /* package private */ void compareRecordTypes(Schema oldSchema, Schema newSchema, List<Message> messages, String name)
  {
    ...
    for(Field newField : newSchema.getFields())
    {
      ...
      if(oldField == null)
      {
        // This is a new field that did not exist in the original schema.
      ...
      else
      {
        ...
        // Check if the field was changed from optional to required or vice versa.
      ...
      // For all fields in the new schema (whether or not it existed in the old schema), if there is a default value for this field, make sure it is legal.
      if(newField.defaultValue() != null)
      {
    ...
    for (Field oldField : oldSchema.getFields())
    {
      if (newSchema.getField(oldField.name()) != null)
      {
        ...
        {
          messages.add(new Message(Level.ERROR, "Cannot add indexing to "
              + oldField.name()
        ...
        {
          messages.add(new Message(Level.ERROR, "Cannot change indexing from "
              + oldIndexType + " to " + newIndexType + " for " + oldField.name()
  ...
  /* package private */ void compareUnionTypes(Schema oldSchema, Schema newSchema, List<Message> messages, String name)
  {
    ...
    // Build a list of type names, ignoring nulls which are checked separately as optional/required fields
  ...
  /* package private */ void compareEnumTypes(Schema oldSchema, Schema newSchema, List<Message> messages, String name)
  {
  ...
  /* package private */ void compareArrayTypes(Schema oldSchema, Schema newSchema, List<Message> messages, String name)
  {
  ...
  /* package private */ void compareMapTypes(Schema oldSchema, Schema newSchema, List<Message> messages, String name)
  {
  ...
  /* package private */ void compareFixedTypes(Schema oldSchema, Schema newSchema, List<Message> messages, String name)
  {
  ...
  /* package private */ void comparePrimitiveTypes(Schema oldSchema, Schema newSchema, List<Message> messages, String name)
  {
    if(oldSchema == null || newSchema == null)
    {
    ...
    if(oldType != newType)
    {
      if(((oldType == Schema.Type.INT && (newType == Schema.Type.LONG || newType == Schema.Type.FLOAT || newType == Schema.Type.DOUBLE)) ||
        (oldType == Schema.Type.LONG && (newType == Schema.Type.FLOAT || newType == Schema.Type.DOUBLE)) ||
        (oldType == Schema.Type.FLOAT && (newType == Schema.Type.DOUBLE))))
      {
  ...
  /**
   * Returns true if this field is optional. Optional fields are represented as a type union containing the null type.
   * @param field
   * @return
   */
   ...
   * @param field
   */
  /* package private */ static void checkDefaultValueIsLegal(Field field, List<Message> messages, String name)
  {
    if(field == null)
    {
    ...
    {
      // Get the type schema. If this is a UNION, the default must be of the leading type
  ...
  /**
   * Check that the default json node is a valid default value. If not, return the expected type as a String.
   */
