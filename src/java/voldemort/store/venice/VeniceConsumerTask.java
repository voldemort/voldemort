...
public class VeniceConsumerTask implements Runnable {
  ...
  public VeniceConsumerTask(VeniceStore store, String topic, int partition, List<String> seedBrokers, int port) {

    this.store = store;

    // Static serialization service for Venice Messages
    serializer = new VeniceSerializer(new VerifiableProperties());
  ...
  public void run() {
    ...
    String clientName = "Voldemort_Venice_" + topic + "_" + partition;
    ...
    while (execute) {
      ...
      while (messageAndOffsetIterator.hasNext()) {
        ...
        try {
          ...
          logger.error("Venice Storage Node with nodeId " + "-1" + " has been corrupted! Exiting...");
          logger.error(e);
          e.printStackTrace();
   ...
   * */
  private void readMessage(String key, VeniceMessage msg) throws VoldemortVeniceException {
    ...
    switch (msg.getOperationType()) {
      ...
      // error
      default:
        throw new VoldemortVeniceException("Invalid operation type submitted: " + msg.getOperationType());
    }
