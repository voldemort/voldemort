...
public class RoutedStore implements Store<ByteArray, byte[]> {
	...
	private static final Logger logger = Logger.getLogger(RoutedStore.class
			.getName());
	 ...
	 * @param innerStores
	 *            The mapping of node to client
	 * @param routingStrategy
	 *            The strategy for choosing a node given a key
	 * @param requiredReads
	 *            The minimum number of reads that must complete before the
	 *            operation will return
	 * @param requiredWrites
	 *            The minimum number of writes that must complete before the
	 *            operation will return
	...
	public RoutedStore(String name,
			...
			Cluster cluster, StoreDefinition storeDef, int numberOfThreads,
			boolean repairReads, long timeoutMs) {
		this(name, innerStores, cluster, storeDef, repairReads, Executors
				.newFixedThreadPool(numberOfThreads), timeoutMs,
				NODE_BANNAGE_MS, SystemTime.INSTANCE);
	}
	...
	public RoutedStore(String name,
			Map<Integer, Store<ByteArray, byte[]>> innerStores,
			Cluster cluster, StoreDefinition storeDef, boolean repairReads,
			ExecutorService threadPool, long timeoutMs, long nodeBannageMs,
			Time time) {
		...
		this.name = name;
		this.innerStores = new ConcurrentHashMap<Integer, Store<ByteArray, byte[]>>(
				innerStores);
		...
		this.routingStrategy = new RoutingStrategyFactory(this.cluster)
				.getRoutingStrategy(storeDef);
	}

	public boolean delete(final ByteArray key, final Version version)
			throws VoldemortException {
		StoreUtils.assertValidKey(key);
		final List<Node> nodes = availableNodes(routingStrategy
				.routeRequest(key.get()));
		...
		final List<Exception> failures = Collections
		...
		for (final Node node : nodes) {
			this.executor.execute(new Runnable() {
				...
				public void run() {
					try {
						boolean deleted = innerStores.get(node.getId()).delete(
								key, version);
		...
		if (this.storeDef.getPreferredWrites() <= 0) {
			...
			for (int i = 0; i < numNodes; i++) {
				try {
					boolean acquired = semaphore.tryAcquire(timeoutMs,
							TimeUnit.MILLISECONDS);
					if (!acquired)
						logger
								.warn("Delete operation timed out waiting for operation "
										+ i
										+ " to complete after waiting "
										+ timeoutMs + " ms.");
					...
					throw new InsufficientOperationalNodesException(
							"Delete operation interrupted!", e);
				}
			}
		}
		...
		if (successes.get() < storeDef.getRequiredWrites())
			throw new InsufficientOperationalNodesException(this.storeDef
					.getRequiredWrites()
	...
	public Map<ByteArray, List<Versioned<byte[]>>> getAll(
			Iterable<ByteArray> keys) throws VoldemortException {
		...
		for (ByteArray key : keys) {
			List<Node> availableNodes = availableNodes(routingStrategy
					.routeRequest(key.get()));

			// quickly fail if there aren't enough nodes to meet the requirement
			checkRequiredReads(availableNodes);
			int preferredReads = storeDef.getPreferredReads();
			List<Node> preferredNodes = Lists
					.newArrayListWithCapacity(preferredReads);
		...
		try {
			...
			futures = executor.invokeAll(callables, timeoutMs * 3,
					TimeUnit.MILLISECONDS);
		} catch (InterruptedException e) {
			throw new InsufficientOperationalNodesException(
					"getAll operation interrupted.", e);
		}
		for (Future<GetAllResult> f : futures) {
			if (f.isCancelled()) {
				logger.warn("Get operation timed out after " + timeoutMs
						+ " ms.");
				continue;
			}
			try {
				...
				for (ByteArray key : getResult.callable.nodeKeys) {
					List<Versioned<byte[]>> retrieved = getResult.retrieved
							.get(key);
			...
			if (successCount < storeDef.getPreferredReads()) {
				...
				if (extraNodes != null) {
					for (Node node : extraNodes) {
						try {
							List<Versioned<byte[]>> values = innerStores.get(
									node.getId()).get(key);
							List<Versioned<byte[]>> versioneds = result
									.get(key);
							if (versioneds == null)
							...
							logger.warn(
									"Error in GET_ALL on node " + node.getId()
											+ "(" + node.getHost() + ")", e);
							failures.add(e);
						}
					}
		...
		for (Map.Entry<ByteArray, List<Versioned<byte[]>>> entry : result
				.entrySet()) {
			if (repairReads && entry.getValue().size() > 1)
				repairReads(nodeValues);
		}

		for (Map.Entry<ByteArray, MutableInt> mapEntry : keyToSuccessCount
				.entrySet()) {
	...
	public List<Versioned<byte[]>> get(final ByteArray key)
		...
		final List<Versioned<byte[]>> retrieved = Collections
				.synchronizedList(new ArrayList<Versioned<byte[]>>());
		final List<NodeValue<ByteArray, byte[]>> nodeValues = Collections
				.synchronizedList(new ArrayList<NodeValue<ByteArray, byte[]>>());
		...
		int attempts = Math
				.min(this.storeDef.getPreferredReads(), nodes.size());
				...
				public void run() {
					try {
						if (logger.isTraceEnabled())
							logger.trace("Attempting get operation on node "
									+ node.getId() + " for key '"
									+ ByteUtils.toHexString(key.get()) + "'.");
						List<Versioned<byte[]>> fetched = innerStores.get(
								node.getId()).get(key);
						retrieved.addAll(fetched);
						if (repairReads) {
							for (Versioned<byte[]> f : fetched)
								nodeValues
										.add(new NodeValue<ByteArray, byte[]>(
												node.getId(), key, f));
						}
						successes.incrementAndGet();
						node.getStatus().setAvailable();
						...
						logger.warn("Error in GET on node " + node.getId()
								+ "(" + node.getHost() + ")", e);
						failures.add(e);
					} finally {
						// signal that the operation is complete
		...
		while (successes.get() < this.storeDef.getPreferredReads()
			...
			try {
				...
				if (repairReads) {
					for (Versioned<byte[]> f : fetched)
						nodeValues.add(new NodeValue<ByteArray, byte[]>(node
								.getId(), key, f));
				}
				successes.incrementAndGet();
				node.getStatus().setAvailable();
		...
		if (logger.isTraceEnabled())
			logger.trace("GET retrieved the following node values: "
					+ formatNodeValues(nodeValues));
		...
		if (successes.get() >= this.storeDef.getRequiredReads())
			...
			throw new InsufficientOperationalNodesException(
					this.storeDef.getRequiredReads() + " reads required, but "
							+ successes.get() + " succeeded.", failures);
	}
	...
	private void repairReads(final List<NodeValue<ByteArray, byte[]>> nodeValues) {
		this.executor.execute(new Runnable() {
			...
			public void run() {
				for (NodeValue<ByteArray, byte[]> v : readRepairer
						.getRepairs(nodeValues)) {
					try {
						if (logger.isDebugEnabled())
							logger.debug("Doing read repair on node "
						...
						if (logger.isDebugEnabled())
							logger
									.debug("Read repair cancelled due to obsolete version on node "
											+ v.getNodeId()
											+ " for key '"
											+ v.getKey()
											+ "' with version "
	...
	private void checkRequiredReads(final List<Node> nodes)
		...
		if (nodes.size() < this.storeDef.getRequiredReads())
			throw new InsufficientOperationalNodesException("Only "
					+ nodes.size() + " nodes in preference list, but "
					+ this.storeDef.getRequiredReads() + " reads required.");
	}

	private String formatNodeValues(
	...
	public void put(final ByteArray key, final Versioned<byte[]> versioned)
		...
		// A list of thrown exceptions, indicating the number of failures
		final Map<Integer, Exception> failures = Collections
				.synchronizedMap(new HashMap<Integer, Exception>(1));
		...
		if (successes.get() < 1)
			throw new InsufficientOperationalNodesException(
					"No master node succeeded!", failures.size() > 0 ? failures
							.get(0) : null);
		else
			currentNode++;
			...
			this.executor.execute(new Runnable() {
				...
				public void run() {
					try {
						...
						logger.warn("Error in PUT on node " + node.getId()
								+ "(" + node.getHost() + ")", e);
						failures.put(node.getId(), e);
					} finally {
						// signal that the operation is complete
		...
		for (int i = 0; i < blockCount; i++) {
			try {
				...
				boolean acquiredPermit = semaphore.tryAcquire(Math.max(
						remainingNs, 0), TimeUnit.NANOSECONDS);
				if (!acquiredPermit) {
					logger.warn("Timed out waiting for put # " + (i + 1)
							+ " of " + blockCount + " to succeed.");
					break;
				}
				// okay, at least the required number of operations have
				// completed, were they successful?
		...
		if (successes.get() < storeDef.getRequiredWrites()) {
			throw new InsufficientOperationalNodesException(successes.get()
					...
					+ this.storeDef.getRequiredWrites() + " are required.",
					failures.values());
		}
	...
	private void markUnavailable(Node node, Exception e) {
		logger.warn("Could not connect to node " + node.getId() + " at "
				+ node.getHost() + " marking as unavailable for "
				+ this.nodeBannageMs + " ms.", e);
		logger.debug(e);
		node.getStatus().setUnavailable();
	}

	private Versioned<byte[]> incremented(Versioned<byte[]> versioned,
			int nodeId) {
		return new Versioned<byte[]>(versioned.getValue(),
				((VectorClock) versioned.getVersion()).incremented(nodeId, time
						.getMilliseconds()));
	}
	...
	private final class GetAllCallable implements Callable<GetAllResult> {
		...
		public GetAllResult call() {
			Map<ByteArray, List<Versioned<byte[]>>> retrieved = Collections
					.emptyMap();
			Exception exception = null;
			List<NodeValue<ByteArray, byte[]>> nodeValues = Lists
					.newArrayList();
			try {
				retrieved = innerStores.get(node.getId()).getAll(nodeKeys);
				if (repairReads) {
					for (Map.Entry<ByteArray, List<Versioned<byte[]>>> entry : retrieved
							.entrySet()) {
						ByteArray key = entry.getKey();
						for (Versioned<byte[]> v : entry.getValue())
							nodeValues.add(new NodeValue<ByteArray, byte[]>(
									node.getId(), key, v));
					}
				}
				node.getStatus().setAvailable();
