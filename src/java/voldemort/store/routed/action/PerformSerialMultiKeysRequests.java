...
public abstract class PerformSerialMultiKeysRequests<T> extends
    ...
    public PerformSerialMultiKeysRequests(MultiKeysPipelineData<T> pipelineData,
                                          Event completeEvent,
    ...
    public abstract T transform(Store<ByteArray, byte[], byte[]> store,
                                Map<ByteArray, T> result,
                                ByteArray key);

    public void execute(Pipeline pipeline) {
        Map<ByteArray, T> result = pipelineData.getResult();
        ...
        for(ByteArray key: keys) {
            ...
            if(logger.isDebugEnabled())
                logger.debug(pipeline.getOperation() + " for key "
            ...
            for(Node node: extraNodes) {
                ...
                try {
                    ...
                    T values = transform(store, result, key);

                    Map<ByteArray, T> map = new HashMap<ByteArray, T>();
                    map.put(key, values);

                    Response<Iterable<ByteArray>, Map<ByteArray, T>> response = new Response<Iterable<ByteArray>, Map<ByteArray, T>>(node,
                                                                                                                                     Arrays.asList(key),
                                                                                                                                     map,
