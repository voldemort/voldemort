...
public class RestServerGetRequestValidator extends RestServerRequestValidator {
	...
	public RestServerGetRequestValidator(HttpRequest request,
			MessageEvent messageEvent, StoreRepository storeRepository) {
	...
	public boolean hasGetVersionRequestHeader() {
		...
		String headerValue = this.request
				.getHeader(RestMessageHeaders.X_VOLD_GET_VERSION);
		if (headerValue != null) {
			result = true;
		}
		return result;
	...
	public CompositeVoldemortRequest<ByteArray, byte[]> constructCompositeVoldemortRequestObject() {
		if (parseAndValidateRequest()) {
			if (this.isGetVersionRequest) {
				this.requestObject = new CompositeGetVersionVoldemortRequest<ByteArray, byte[]>(
						this.parsedKeys.get(0), this.parsedTimeoutInMs,
						this.parsedRequestOriginTimeInMs,
						this.parsedRoutingType);

			} else if (this.parsedKeys.size() > 1) {
				this.requestObject = new CompositeGetAllVoldemortRequest<ByteArray, byte[]>(
						this.parsedKeys, this.parsedTimeoutInMs,
						this.parsedRequestOriginTimeInMs,
						this.parsedRoutingType);
			} else {
				this.requestObject = new CompositeGetVoldemortRequest<ByteArray, byte[]>(
						this.parsedKeys.get(0), this.parsedTimeoutInMs,
						this.parsedRequestOriginTimeInMs,
						this.parsedRoutingType);
			}
