...
public class Db4oStorageConfiguration implements StorageConfiguration {
    ...
    private final Map<String, Db4oStorageEngine> stores = Maps.newHashMap();
    ...
    public StorageEngine<ByteArray, byte[]> getStore(String storeName) {
        synchronized(lock) {
            Db4oStorageEngine store = stores.get(storeName);
            if(store != null)
            ...
            try {
                ...
                Db4oStorageEngine engine = new Db4oStorageEngine(storeName,
                                                                 environment,
                                                                 db,
