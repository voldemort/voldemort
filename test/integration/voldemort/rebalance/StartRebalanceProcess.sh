...
cd $VLDMDIR
bin/voldemort-rebalance.sh --current-cluster ${TESTCFG_PREFIX}1/config/cluster.xml --current-stores ${TESTCFG_PREFIX}1/config/stores.xml --target-cluster $WORKDIR/$ENDMETADATA --output-dir $WORKDIR > $LOGDIR/$LOGFILE 2>&1 &
