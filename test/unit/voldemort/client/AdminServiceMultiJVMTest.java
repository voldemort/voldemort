...
public class AdminServiceMultiJVMTest extends TestCase {
    ...
    // TODO : fix me : blocked on Kirk ec2-testing fixes.
    ...
    // private static String clusterXmlfile =
    // "test/common/voldemort/config/three-node-cluster.xml";
    //
    // VoldemortClusterStarterApp starterApp = new VoldemortClusterStarterApp();
    //
    ...
    // Node node0 = new Node(0, "localhost", ports[0], ports[1], ports[2],
    // Arrays.asList(0, 1));
    //
    // ports = ServerTestUtils.findFreePorts(3);
    // Node node1 = new Node(1, "localhost", ports[0], ports[1], ports[2],
    // Arrays.asList(2, 3));
    //
    ...
    // client.updateRemoteCluster(server.getIdentityNode().getId(),
    // updatedCluster);
    //
    // assertEquals("Cluster should match", updatedCluster,
    // server.getMetadataStore().getCluster());
    ...
    // server.getMetadataStore().getServerState();
    ...
    // server.getStoreRepository().getStorageEngine(storeName);
    // assertNotSame("Store '" + storeName + "' should not be null", null,
    // store);
    //
    ...
    // }
    //
    // getAdminClient().deletePartitions(0, storeName, Arrays.asList(0, 2),
    // null);
    //
    // RoutingStrategy routingStrategy =
    // server.getMetadataStore().getRoutingStrategy(storeName);
    // for(Pair<ByteArray, Versioned<byte[]>> entry: entrySet) {
    ...
    // } else {
    // assertEquals("store should have all 1,3 entries", 1,
    // store.get(entry.getFirst())
    // .size());
    ...
    // client.fetchPartitionKeys(server.getIdentityNode().getId(),
    // storeName,
    // Arrays.asList(0, 1),
    // null);
    ...
    // || routingStrategy.getPartitionList(entry.getFirst().get()).contains(1))
    // {
    // expected.put(new String(entry.getFirst().get()), new
    // String(entry.getSecond()
    // .getValue()));
    //
    // }
    // }
    ...
    //
    // AdminClient client = getAdminClient();
    // Iterator<Pair<ByteArray, Versioned<byte[]>>> fetchIt =
    // client.fetchPartitionEntries(server.getIdentityNode()
    // .getId(),
    // storeName,
    // Arrays.asList(0,
    // 1),
    ...
    // AdminClient client =
    // ServerTestUtils.getAdminClient(server2.getIdentityNode(),
    ...
    // Store<ByteArray, byte[]> store2 =
    // server2.getStoreRepository().getStorageEngine(storeName);
    // assertNotSame("Store '" + storeName + "' should not be null", null,
    // store2);
    //
    // StoreDefinition storeDef =
    // server.getMetadataStore().getStoreDef(storeName);
    // assertNotSame("StoreDefinition for 'users' should not be null", null,
    // storeDef);
    ...
    // && new String(value).equals(new
    // String(store2.get(key).get(0).getValue()))) {
    // matched++;
    // }
    // }
    // }
    ...
    // Set<Pair<ByteArray, Versioned<byte[]>>> entrySet = new
    // HashSet<Pair<ByteArray, Versioned<byte[]>>>();
    //
    // for(int i = 0; i <= 1000; i++) {
    // ByteArray key = new ByteArray(ByteUtils.getBytes("" + i, "UTF-8"));
    // Versioned<byte[]> value = new
    // Versioned<byte[]>(ByteUtils.getBytes("value-" + i,
    // "UTF-8"));
